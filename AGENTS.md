...
- HA selectors: when using `selector.select`, `options` should be simple strings (`- off`) or full `label`/`value` pairs. Avoid mixed/partial structures; malformed options will cause import errors.
