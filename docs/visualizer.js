...
(function() {
    ...
    const BLUEPRINT_URLS = {
        'adaptive-comfort-control': 'https://raw.githubusercontent.com/schoolboyqueue/home-assistant-blueprints/main/blueprints/adaptive-comfort-control/adaptive_comfort_control_pro_blueprint.yaml',
        'bathroom-light-fan-control': 'https://raw.githubusercontent.com/schoolboyqueue/home-assistant-blueprints/main/blueprints/bathroom-light-fan-control/bathroom_light_fan_control_pro.yaml',
        'multi-switch-light-control': 'https://raw.githubusercontent.com/schoolboyqueue/home-assistant-blueprints/main/blueprints/multi-switch-light-control/multi_switch_light_control_pro.yaml',
        'adaptive-shades': 'https://raw.githubusercontent.com/schoolboyqueue/home-assistant-blueprints/main/blueprints/adaptive-shades/adaptive_shades_pro.yaml',
        'adaptive-fan-control': 'https://raw.githubusercontent.com/schoolboyqueue/home-assistant-blueprints/main/blueprints/adaptive-fan-control/adaptive_fan_control_pro_blueprint.yaml'
    };
    ...
    const BLUEPRINT_SUMMARIES = {
        'adaptive-comfort-control': {
            ...
            mainFeatures: ['Smart temperature control', 'Humidity management', 'Occupancy-aware', 'Energy efficient scheduling'],
        ...
        'bathroom-light-fan-control': {
            ...
            mainFeatures: ['Motion-activated lights', 'Humidity-triggered fan', 'Night mode dimming', 'Wasp-in-a-Box occupancy'],
    ...
    const TRIGGER_FRIENDLY_NAMES = {
        // Bathroom Light Fan Control
        'ha_start': { name: 'Home Assistant Starts', icon: '🏠', description: 'When Home Assistant restarts, check current state' },
        'humidity_bath_change': { name: 'Bathroom Humidity Changes', icon: '💧', description: 'When bathroom humidity sensor updates' },
        'humidity_home_change': { name: 'Home Humidity Changes', icon: '💧', description: 'When baseline humidity changes' },
        'light_manual_off': { name: 'Light Turned Off Manually', icon: '💡', description: 'When someone manually turns off the light' },
        'fan_max_runtime_expired': { name: 'Fan Max Runtime Reached', icon: '⏱️', description: 'Fan has run for maximum allowed time' },
        'lights_off_for_fan_auto_off': { name: 'Lights Off Delay Passed', icon: '💨', description: 'Lights have been off long enough to consider turning off fan' },
        ...
        'down_single_z2m': { name: 'Down Button Pressed Once', icon: '🔽', description: 'Single press on down (Zigbee2MQTT)' },
        'up_double_z2m': { name: 'Up Button Double-Pressed', icon: '⏫', description: 'Double-press on up (Zigbee2MQTT)' },
        'down_double_z2m': { name: 'Down Button Double-Pressed', icon: '⏬', description: 'Double-press on down (Zigbee2MQTT)' },
        'up_triple_z2m': { name: 'Up Button Triple-Pressed', icon: '🔼', description: 'Triple-press on up (Zigbee2MQTT)' },
        'down_triple_z2m': { name: 'Down Button Triple-Pressed', icon: '🔽', description: 'Triple-press on down (Zigbee2MQTT)' },
        ...
        'lutron_favorite': { name: 'Pico Favorite Button Pressed', icon: '⭐', description: 'Pressed the favorite/center button' },

        // Multi-Switch - State sync
        'state_sync': { name: 'Light State Changed Externally', icon: '🔄', description: 'Light was controlled by another source' },
        'state_sync_light_on': { name: 'Light Turned On Externally', icon: '💡', description: 'Light was turned on by another automation or app' },
        'state_sync_light_off': { name: 'Light Turned Off Externally', icon: '🔌', description: 'Light was turned off by another automation or app' },
    ...
    function generateTemplateConditionDescription(template) {
        ...
        const keywords = clean.match(/\b(motion|door|light|fan|humidity|temperature|presence|occupied|home|night|day|sun|time)\b/gi);
        ...
        Object.entries(triggerGroups).forEach(([category, triggers]) => {
            triggers.forEach((trigger, i) => {
                ...
                if (conditions.length > 0) {
                    const condLabel = conditions.slice(0, 2).map(c => `✓ ${c}`).join('\\n');
    ...
    function groupTriggersByCategory(triggers) {
        ...
        return Object.fromEntries(
            Object.entries(groups).filter(([_, triggers]) => triggers.length > 0)
        );
    }
    ...
    function inferGroupNameFromActions(actions) {
        ...
        if (actionStr.includes('turn on fan') || actionStr.includes('fan.turn_on') || actionStr.includes('switch.turn_on')) {
            return 'Turn on fan';
        }
        if (actionStr.includes('turn off fan') || actionStr.includes('fan.turn_off') || actionStr.includes('switch.turn_off')) {
            return 'Turn off fan';
        }
    ...
    function extractMeaningfulActions(actions, depth = 0) {
        ...
        for (const action of actions) {
            ...
            if (action.type === 'service') {
                ...
                if (!svc.includes('logbook') && !svc.includes('input_boolean') && !svc.includes('input_datetime') && action.description) {
    ...
    function inferWhyForTrigger(triggerId, actions) {
        ...
        if (id.includes('single') || id.includes('double') || id.includes('triple') || id.includes('hold') || id.includes('release')) {
