...
# Include this file in project-specific Makefiles and override as needed
#
...
#   GO_LOCAL_PATH  - Local import path for goimports (e.g., github.com/org/repo)
#
# Optional variables (have defaults):
#   VERSION        - Version string (default: dev)
#   BUILD_DIR      - Output directory for builds (default: build)
#   BUILD_PATH     - Go package path to build (default: .)
#   GOLANGCI_LINT_VERSION - Version of golangci-lint (default: v1.62.2)
#   GOFUMPT_VERSION       - Version of gofumpt (default: v0.7.0)
...
imports:
	...
	@goimports -local $(GO_LOCAL_PATH) -w .
...
format-check:
	...
	@test -z "$$(goimports -local $(GO_LOCAL_PATH) -l . 2>&1)" || (echo "$(RED)Imports are not properly organized. Run 'make format' to fix.$(NC)" && exit 1)
...
build:
	...
	@mkdir -p $(BUILD_DIR)
	CGO_ENABLED=0 $(GOBUILD) $(LDFLAGS) -o $(BUILD_DIR)/$(BINARY_NAME) $(BUILD_PATH)
...
build-race:
	...
	@mkdir -p $(BUILD_DIR)
	$(GOBUILD) -race $(LDFLAGS) -o $(BUILD_DIR)/$(BINARY_NAME)-race $(BUILD_PATH)

# Build for Linux AMD64 (typical server/container)
.PHONY: build-linux-amd64
build-linux-amd64:
	@mkdir -p $(BUILD_DIR)
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 $(GOBUILD) $(LDFLAGS) -o $(BUILD_DIR)/$(BINARY_NAME)-linux-amd64 $(BUILD_PATH)

# Build for Linux ARM64 (Raspberry Pi 4, newer 64-bit Pi)
.PHONY: build-linux-arm64
build-linux-arm64:
	@mkdir -p $(BUILD_DIR)
	CGO_ENABLED=0 GOOS=linux GOARCH=arm64 $(GOBUILD) $(LDFLAGS) -o $(BUILD_DIR)/$(BINARY_NAME)-linux-arm64 $(BUILD_PATH)
...
build-darwin-amd64:
	@mkdir -p $(BUILD_DIR)
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 $(GOBUILD) $(LDFLAGS) -o $(BUILD_DIR)/$(BINARY_NAME)-darwin-amd64 $(BUILD_PATH)

# Build for macOS ARM64 (Apple Silicon)
.PHONY: build-darwin-arm64
build-darwin-arm64:
	@mkdir -p $(BUILD_DIR)
	CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 $(GOBUILD) $(LDFLAGS) -o $(BUILD_DIR)/$(BINARY_NAME)-darwin-arm64 $(BUILD_PATH)

# Build for Windows AMD64
.PHONY: build-windows-amd64
build-windows-amd64:
	@mkdir -p $(BUILD_DIR)
	CGO_ENABLED=0 GOOS=windows GOARCH=amd64 $(GOBUILD) $(LDFLAGS) -o $(BUILD_DIR)/$(BINARY_NAME)-windows-amd64.exe $(BUILD_PATH)
...
#------------------------------------------------------------------------------
# Help (base implementation - projects should extend with help-extra)
#------------------------------------------------------------------------------
...
# Default help target - projects can override this
.PHONY: help
help: help-base
